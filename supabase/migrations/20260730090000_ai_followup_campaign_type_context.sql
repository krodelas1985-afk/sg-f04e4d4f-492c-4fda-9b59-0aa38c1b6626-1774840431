-- AI Follow-Up: expose campaign_type in the decision context bundle.
--
-- W6's "Build Decision Request" node needs to branch its system prompt between
-- buyer campaigns and BaMo's own B2B client-acquisition campaign
-- (campaigns.campaign_type = 'bamo_b2b'), mirroring the b2bDecisionPrompt /
-- buyerDecisionPrompt split that already exists in W2. The bundle previously
-- carried only campaign.target_action + campaign.tone, so the Brain could not
-- tell a homebuyer from a broker being recruited.
--
-- Additive: one extra key inside the existing 'campaign' object. Signature and
