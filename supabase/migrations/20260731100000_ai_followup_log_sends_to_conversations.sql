-- W6 sent Messenger messages via the FB Graph API but never wrote them to
-- conversations: prospects received messages that existed nowhere in the CRM,
-- leads.last_outbound_at never advanced, and the next decision's recent_messages
-- was blind to its own prior follow-up.
--
-- The insert belongs in this function rather than a new n8n node - it already
-- receives the message text and is only reached after the FB send succeeded.
-- Verified against every trigger on conversations: the unread counter and all
-- three enrollment-exit triggers are inbound-gated.
--
-- NOTE for anyone backfilling historic sends: update_lead_last_message_at sets
-- last_message_at unconditionally (no GREATEST), so inserting rows with old
-- created_at values drags last_message_at backwards. Repair it afterwards from
...
BEGIN
  ...
  -- the message actually went out to Messenger; the CRM must show it
