-- Add lead_type to the leads list RPC: return the column and add a p_lead_type filter.
-- Return-shape change requires DROP + CREATE. p_lead_type is appended as an optional
-- trailing arg so existing named-arg callers keep working.
DROP FUNCTION IF EXISTS public.get_leads_with_details(
  uuid, integer, integer, text, text, text, uuid, uuid, text, text, text, text);
...
BEGIN
  ...
  SELECT
    ...
    l.buyer_type, l.lead_type, l.next_follow_up_date, l.last_contacted_at,
    l.last_inbound_at,
